...
def message(argument2):
    switcher2 = {
        1: "Use authorized Mastercard to add tag                    ",
        2: "Input Volume                                            ",
        3: "Volume should be <= remaining allotted quota            ",
...
class Main(object):
    ...
    def __init__(self, master):
        ...
        # for excess input
        self.excess_lock = threading.RLock()

        # lock for self.serv_receive
        self.serv_receive_lock = threading.RLock()
        ...
        self.excess = False
        self.serv_receive = False

        # variables
        self.present_seneca = 0
        self.present_volume_litre = 0
        self.volume_input = 0
        ...
        # initial labels
        self.networkLB.set(f"SSID: {self.network}")  # WiFi connected to
        self.messageLB.set(self.message)  # real-time message display/notification
        self.UIDLB.set(f"Plate No.: {self.asset_name}")  # display the asset name which is the plate number
        self.rem_VolLB.set(f"Rem. Vol.: {round(self.rem_Vol, 2)}")  # display remaining volume here as received from the mqtt server
        self.volumeLB.set(f"Volume: {str(self.present_volume_litre)}")  # remember to make this realtime volume
        self.readerStatusLB.set(f"Reader Status: {self.readerStatus}")  # tag reader
        self.tagIDLB.set(f"Tag ID: {self.rfid}")  # displays the rfid. refresh this when necessary
        self.dateTimeLB.set(f"Date Time: {self.dt}")  # date-time
        self.communicationLB.set(f"Communication: {self.communication}")  # communication status
        self.statusLB.set(f"Status: {self.status}")  # current status
        ...
        self.totalizerLB.set(f"Pump totalizer: {round(self.asset_totalizer, 2)}")  # final totalizer
    ...
    def query_mt124(self):
        ...
        while True:
            ...
            if not self.tag_present and not self.low_power:
                with self.trans_complete_lock:
                    self.trans_complete_flag = False
    ...
    def print_value(self):
        ...
        if 0 < self.volume_input <= self.rem_Vol and self.tag_registered_flag and not self.fuel_timeout_flag2:
            with self.seneca_flag_lock:
    ...
    def popup_thread(self):
        while True:
            if not self.popup_flag and not self.low_power and self.i and self.tag_registered_flag:
            ...
            if int(time.perf_counter()) - self.popup_time >= self.POPUP_TIMEOUT and self.popup_flag and not self.tag_present:
                self.inputField.delete(0, 'end')
                self.inputField.destroy()
    ...
    def scan_mastercard(self):
        ...
        while True:
            if self.scan_card_flag and not self.low_power:
    ...
    def seneca(self):
        while True:
            ...
            if not self.seneca_flag and not self.low_power:  # if the seneca flag is False. This is the default state.  It is set true after volume has been input before relay is activated
                try:
            ...
            if self.fuel_dispense_flag:
                ...
                # if the present seneca reading minus the last seneca reading is less than 100, then turn off the relay and reset parameters
                    ...
                    msg_str = f'{{"asset_code":"{self.rfid}", "used_vol":"{self.rem_Vol}", "pump_id":"002", "fill_qty":"{round(self.present_volume_litre, 2)}", "status":"completed", "trnx_dt_time":"{self.dt}", "title":"asset_trnx", "cmd_type":"query"}}'
                ...
                if int(time.perf_counter()) - self.fuel_record_time1 >= self.FUEL_TIMEOUT:
                    ...
                    if self.present_seneca - self.last_seneca < self.SENECA_DIFFERENCE and time.perf_counter() - self.seneca_record_time > 0.7:
                        ...
                        msg_str2 = f'{{"asset_code":"{self.rfid}", "pump_id":"002", "fill_qty":"{round(self.present_volume_litre, 2)}", "status":"completed", "trnx_dt_time":"{self.dt}", "title":"asset_trnx", "cmd_type":"query"}}'
                        if 0 < self.present_volume_litre < self.rem_Vol:
        ...
        while True:
            ...
            if self.low_power and not self.fuel_dispense_flag:
                ...
                if self.asset_code == self.rfid and self.asset_status == 'registered':
                    ...
                    if self.rem_Vol > 0 and not self.dest_flag and not self.i:
                        with self.popup_flag_lock:
                            self.popup_flag = False
                ...
                if (time.perf_counter() - self.read_time) > self.READ_TAG_TIMEOUT:
                    ...
                    if 0 < self.present_volume_litre < self.rem_Vol and self.fuel_dispense_flag:
                        msg_str = f'{{"asset_code":"{self.rfid}", "pump_id":"002", "fill_qty":"{round(self.present_volume_litre, 2)}", "status":"completed", "trnx_dt_time":"{self.dt}", "title":"asset_trnx", "cmd_type":"query"}}'
                        self.mqtt.client.publish(self.mqtt.MQTT_PUB_TOPIC, msg_str)
                        self.communication = "Data upload successful                                 "
                    # print("time elapsed without authentication. diff: " + str(time.perf_counter() - self.read_time))

                if self.fuel_dispense_flag and not self.fuel_timeout_flag2:
                    GPIO.output(RELAY, GPIO.HIGH)
                    # print("______________dispensing fuel.............")

                elif not self.fuel_dispense_flag:
                    GPIO.output(RELAY, GPIO.LOW)

                # if a card is scanned and status is unregistered

                if self.tag_present:
                    if self.scan_card_flag:
                        with self.f_lock:
                            self.f = 1
                    ...
                    elif not self.low_power and not self.popup_flag and self.asset_code == self.rfid and self.asset_status == 'registered' and not self.excess:
                        with self.f_lock:
                            self.f = 12

                    elif self.asset_status == 'unregistered':
                        with self.scan_card_flag_lock:
                    ...
                    elif self.fuel_dispense_flag and not self.ongoing_trans_flag:
                        with self.f_lock:
                            self.f = 18
                            self.communication = "                                                  "

                    elif self.trans_complete_flag:
                        with self.f_lock:
                            self.f = 19

                    elif not self.trans_complete_flag and self.ongoing_trans_flag:
                        with self.f_lock:
                            self.f = 20
                    ...
                    elif self.mqtt_auth_flag and not self.serv_receive:
                        with self.f_lock:
                            self.f = 8

                        with self.f_lock:
                            if not self.popup_flag and not self.excess:
                                self.f = 7
                elif not self.tag_present:
                    if not self.intrpt_flag:
                        with self.f_lock:
                            self.f = 10

            self.message = message(self.f)
